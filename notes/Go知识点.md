...
slice底层数据结构是由数组、len、cap组成，所以，在使用append()扩容时，会查看数组后面有没有连续内存快，有就在后面添加，没有就重新生成一个大的数组，新数组容量是原来的2倍(当前切片容量等于 1024并用完的是后,append操作 就会每次增加 25% 的容量，直到新容量大于期望容量)
...
slice:  
- slice的底层确实引用一个数组对象。一个slice由三个部分构成：指针、长度和容量。指针指向第一个slice元素对应的底层数组元素的地址
- 修改slice会修改底层的数组元素
- 扩容会引用一个新的底层数组
