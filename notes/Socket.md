...
比如一个设置了非阻塞的socket有数据来临，触发EPOLLIN，socket对应的读缓冲区有6个字节的数据，read()读取5个字节，直接返回。下次还会触发EPOLLIN，然后可以继续读取剩下的字节。
...
如果使用多线程来应对大量连接，“上下文切换”成本，还有编程模型复杂、数据竞争、同步等问题，写出正确、快速的多线程程序并不是一件容易的事情。所以 Nginx 就选择了单线程的方式，带来的好处就是开发简单，没有互斥锁的成本，减少系统消耗。Nginx 里使用的 epoll，把多个连接的处理，都“复用”到一个单线程里，不按照先来后到的顺序处理，而是只当连接上真正可读、可写的时候才处理，如果可能发生阻塞就立刻切换出去，处理其他的连接。

通过这种方式，Nginx 就完全消除了 I/O 阻塞，把 CPU 利用得“满满当当”，又因为网络收发并不会消耗太多 CPU 计算能力，也不需要切换进程、线程，所以整体的 CPU 负载是相当低的。epoll可以多路复用以外还有一个特点，大量的连接管理工作都是在内核里做的，这就减轻了应用程序的负担，所以 Nginx 可以为每个连接只分配很小的内存维护状态，即使有几万、几十万的并发连接也只会消耗几百 M 内存。
